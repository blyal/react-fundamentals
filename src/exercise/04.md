...
- Can do reusable normal functions that take props objects with a children
  property and return JSX
- Then: can use these functions inside React.createElement as the type (first
  argument) instead of say 'div' – e.g. React.createElement(message, {children:
  "hello"})
- Better: can use < RegularJSX />, e.g. < Message children="hello" /> (just need
  to capitalise the function name)
- That JSX gets compiled into React.createElement(Message, {children: "hello"}),
  or null second argument and "hello" third argument – see earlier exercises and
  next bullet point
- < Message children="hello"> === < Message >hello</ Message> (both compile to
  React.createElement(Message, null, "hello"))
- In the past I just learnt < RegularJSX />, but now I've learned that this is
  actually logically further down the chain of evolution
- PropTypes can be manually added in by a validator function (for custom error
  messages), but they are only for development (auto–removed for production)
- PropTypes also just available in a script tag via Unpkg (all npm packages are)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=alexblyth%40pm.me).
