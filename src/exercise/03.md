...
- JSX is converted to regular React APIs using a code compiler
- BabelJS is one such code compiler
- We can even convert the code on the fly in the browser, using babeljs in
  script tags
- But usually the code is converted at build time when we ship the application
  to the browser
- Can put text content inside JSX tag by simply adding a children prop (e.g.
  children="Hello World)
- Objects with duplicate keys use the last duplicate key
  - This can be useful for default values combined with spread syntax (e.g. if
    {...props} is missing a className, then a preceding className="default"
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=alexblyth%40pm.me).
