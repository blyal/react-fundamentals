...
- React can be declarative in the same manner as pure JS
- We don't use that declarative "raw api" syntax often
- Unpkg is a service which enables us to use npm packages in a normal script tag
- React is separate from ReactDOM (which renders React into the DOM)
- React can also be used with React Native, for example, as opposed to ReactDOM
- React.createElement takes 2+ arguments: elementType, props, children, more
  children etc.
- Children can also be an array inside the props instead (or a single element
  inside the props if there is only one child)
- Knowing this all is useful for building custom components
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=alexblyth%40pm.me).
