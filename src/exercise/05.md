...
- Click on something in DevTools and it becomes the value of "$0" variable,
  which we can then explore in the console
- JSX contains DOM properties rather than HTML attributes (className vs class)
- Good trick: don't deconstruct props if all props always go into one child
  element. Can just pass in "props" and then spread the props into that element
- But we can also deconstruct and spread props as we pass them in, and then pass
  extra unexpected props when we use the component
- Obvious, but good to look for ways to move functionality up to the function
  from the instance – but also to allow them to be overridden
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=alexblyth%40pm.me).
